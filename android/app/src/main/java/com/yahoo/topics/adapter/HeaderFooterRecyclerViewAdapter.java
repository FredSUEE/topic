 ...
 * <p/>
 * Created by hongjiedong on 6/18/15.
 */
public abstract class HeaderFooterRecyclerViewAdapter<VH extends RecyclerView.ViewHolder>
        extends RecyclerView.Adapter<VH> {
    private static final int VIEW_TYPE_MAX_COUNT = 1000;
    private static final int EMPTY_VIEW_TYPE_OFFSET = 0;
    private static final int HEADER_VIEW_TYPE_OFFSET = 1;
    private static final int FOOTER_VIEW_TYPE_OFFSET =
            HEADER_VIEW_TYPE_OFFSET + VIEW_TYPE_MAX_COUNT;
    private static final int CONTENT_VIEW_TYPE_OFFSET =
            FOOTER_VIEW_TYPE_OFFSET + VIEW_TYPE_MAX_COUNT;
    ...
    public final VH onCreateViewHolder(ViewGroup parent, int viewType) {
        if (viewType == EMPTY_VIEW_TYPE_OFFSET) {
            return onCreateEmptyItemViewHolder(parent);
        }
        // Delegate to proper methods based on the viewType ranges.
        else if (viewType >= HEADER_VIEW_TYPE_OFFSET &&
    ...
    public final void onBindViewHolder(VH viewHolder, int position) {
        if (getRealItemCount() == 0 && hasEmptyView() && position == EMPTY_VIEW_TYPE_OFFSET) {
            onBindEmptyItemViewHolder(viewHolder);
        }
        // Delegate to proper methods based on the viewType ranges.
        else if (headerItemCount > 0 && position < headerItemCount) {
    ...
    public final int getItemCount() {
        ...
        int total = getRealItemCount();
        return total == 0 && hasEmptyView() ? 1 : total;
    }
    ...
    public final int getItemViewType(int position) {
        if (getRealItemCount() == 0 && hasEmptyView()) {
            return EMPTY_VIEW_TYPE_OFFSET;
        }
        ...
        if (headerItemCount > 0 && position < headerItemCount) {
            ...
            return validateViewType(
                    getFooterItemViewType(position - headerItemCount - contentItemCount)) +
                    FOOTER_VIEW_TYPE_OFFSET;
        }
    }

    /**
     * @param position position of the item in the raw list
     * @return return true if position is showing content type item
     */
    ...
    private int validateViewType(int viewType) {
        if (viewType < 0 || viewType >= VIEW_TYPE_MAX_COUNT) {
            throw new IllegalStateException(
                    "viewType must be between 0 and " + VIEW_TYPE_MAX_COUNT);
        }
        return viewType;
    }
    ...
    public final void notifyHeaderItemRangeInserted(int positionStart, int itemCount) {
        ...
        if (positionStart < 0 || itemCount < 0 || positionStart + itemCount > headerItemCount) {
            throw new IndexOutOfBoundsException(
                    "The given range [" + positionStart + " - " + (positionStart + itemCount - 1) +
                            "] is not within the position bounds for header items [0 - " +
                            (headerItemCount - 1) + "].");
        }
    ...
    public void notifyHeaderItemMoved(int fromPosition, int toPosition) {
        if (fromPosition < 0 || toPosition < 0 || fromPosition >= headerItemCount ||
            ...
            throw new IndexOutOfBoundsException(
                    "The given fromPosition " + fromPosition + " or toPosition " + toPosition +
                            " is not within the position bounds for header items [0 - " +
                            (headerItemCount - 1) + "].");
        }
    ...
    public final void notifyFooterItemRangeInserted(int positionStart, int itemCount) {
        ...
        notifyItemRangeInserted(positionStart + newHeaderItemCount + newContentItemCount,
                itemCount);
    }
    ...
    public final void notifyFooterItemRemoved(int position) {
        if (position < 0 || position >= footerItemCount) {
            ...
            throw new IndexOutOfBoundsException("The given position " + position +
                    " is not within the position bounds for footer items [0 - " +
                    (footerItemCount - 1) + "]. c: " + contentItemCount);
        }
    ...
    protected abstract VH onCreateEmptyItemViewHolder(ViewGroup parent);

    protected abstract void onBindEmptyItemViewHolder(VH contentViewHolder);
